...
def main():
    ...
    if os.path.exists(f'{output_path}/live_hosts'):
        ...
        xml_result += '</nmaprun>'
        with open(f'{output_path}/exmap_output.xml', 'w+') as exmap_output:
            exmap_output.write(xml_result)
        print('\x1b[33m' + f'\nResults written to {output_path}/exmap_output.xml' + '\x1b[0m')
